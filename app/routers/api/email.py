...
async def get_email(id_email: int,
...
async def criar_email(email: EmailCriarAtualizar,
                    repo: EmailRepository = Depends(get_email_repository)):
    return await repo.criar_email(email)

@router.put("/{id_email}", response_model=Email)
async def update_email(id_email: int, email: EmailCriarAtualizar,
                    repo: EmailRepository = Depends(get_email_repository)):
...
async def delete_email(id_email: int,
                    repo: EmailRepository = Depends(get_email_repository)):
